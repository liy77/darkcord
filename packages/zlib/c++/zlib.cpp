...
namespace zlib {
std::vector<bool>
ValidateArgs(const v8::FunctionCallbackInfo<v8::Value> &args) {
...
void inflateSyncBuffer(v8::Isolate *isolate, int flush, std::size_t windowBits,
                       std::size_t chunkSize, std::size_t inputLength,
                       char *input, v8::Local<v8::Object> &result) {
  z_stream strm;
  ...
  if (ret != Z_OK) {
    isolate->ThrowException(v8::Exception::Error(v8::String::NewFromUtf8Literal(
        isolate, "Failed to initialize zlib stream")));
    return;
  }
  ...
  v8::Local<v8::Object> outputBuffer =
      node::Buffer::New(isolate, chunkSize).ToLocalChecked();
  ...
  while (true) {
    ...
    if (outputLength > chunkSize) {
      ...
      return inflateSyncBuffer(isolate, flush, windowBits, chunkSize * 2,
                               inputLength, input, result);
    }

    outputLength += (chunkSize - strm.avail_out);
    std::memcpy(output + outputLength - (chunkSize - strm.avail_out), dest,
                chunkSize - strm.avail_out);

    if (strm.avail_out == 0) {
      outputLength += chunkSize;
      v8::Local<v8::Object> newOutputBuffer =
          node::Buffer::New(isolate, outputLength + chunkSize).ToLocalChecked();
      std::memcpy(node::Buffer::Data(newOutputBuffer), output,
                  outputLength - chunkSize);
    ...
    if (ret != Z_OK) {
      ...
      isolate->ThrowException(
          v8::Exception::Error(v8::String::NewFromUtf8Literal(
              isolate, "Failed to end inflate stream")));
      return;
    }
  }
...
void deflateSyncBuffer(v8::Isolate *isolate, int flush, std::size_t windowBits,
                       std::size_t level, std::size_t memoryLevel,
                       std::size_t strategy, std::size_t inputLength,
                       char *input, v8::Local<v8::Object> &result) {
  z_stream strm;
  std::memset(&strm, 0, sizeof(z_stream));
  int ret = deflateInit2(&strm, level, Z_DEFLATED, windowBits + 16, memoryLevel,
                         strategy);
...
void InflateSync(const v8::FunctionCallbackInfo<v8::Value> &args) {
    ...
    if (!args[1]->IsInt32()) {
      isolate->ThrowException(
          v8::Exception::TypeError(v8::String::NewFromUtf8Literal(
              isolate, "The flush must be an integer")));
      return;
    }
  ...
  if (validArgs[2]) {
    if (!args[2]->IsInt32()) {
      isolate->ThrowException(
          v8::Exception::TypeError(v8::String::NewFromUtf8Literal(
              isolate, "The window bits must be an integer")));
      return;
    }
  ...
  if (validArgs[3]) {
    if (!args[3]->IsInt32()) {
      isolate->ThrowException(
          v8::Exception::TypeError(v8::String::NewFromUtf8Literal(
              isolate, "The chunk size must be an integer")));
      return;
    }
    ...
    if (chunkSize <= 0) {
      isolate->ThrowException(
          v8::Exception::TypeError(v8::String::NewFromUtf8Literal(
              isolate, "The chunk size must be greater than 0")));
      return;
    }
  }
  ...
  inflateSyncBuffer(isolate, flush, windowBits, chunkSize, inputLength, input,
                    newOutputBuffer);
...
void DeflateSync(const v8::FunctionCallbackInfo<v8::Value> &args) {
  ...
  if (buf->IsString()) {
    ...
    v8::Local<v8::Object> buffer =
        node::Buffer::New(isolate, length).ToLocalChecked();
  ...
  v8::Local<v8::Object> inputBuffer{
      buf->ToObject(isolate->GetCurrentContext()).ToLocalChecked()};
  ...
  if (validArgs[2]) {
    if (!args[2]->IsInt32()) {
      isolate->ThrowException(
          v8::Exception::TypeError(v8::String::NewFromUtf8Literal(
              isolate, "The level must be an integer")));
      return;
    }
  ...
  if (validArgs[3]) {
    if (!args[3]->IsInt32()) {
      isolate->ThrowException(
          v8::Exception::TypeError(v8::String::NewFromUtf8Literal(
              isolate, "The strategy must be an integer")));
      return;
    }
  ...
  if (validArgs[5]) {
    if (!args[5]->IsInt32()) {
      isolate->ThrowException(
          v8::Exception::TypeError(v8::String::NewFromUtf8Literal(
              isolate, "The memory level must be an integer")));
      return;
    }
  ...
  v8::Local<v8::Object> outputBuffer;
  deflateSyncBuffer(isolate, flush, windowBits, level, memoryLevel, strategy,
                    inputLength, input, outputBuffer);
...
public:
  ProcessSync(const char *method, std::size_t windowBits, std::size_t chunkSize,
              int defaultFlush)
      : chunkSize(chunkSize), windowBits(windowBits),
        defaultFlush(defaultFlush), method(method) {}
  ...
  static void Push(const v8::FunctionCallbackInfo<v8::Value> &args) {
    ...
    if (strcmp(method, "inflateRaw") == NULL) {
      // Setting windowBits to -47 to allow inflate raw  (-15 Allow Raw, -32
      // Value to convert the windowBits)
      wb = -47;
    } else if (strcmp(method, "deflateRaw") == NULL) {
      // Setting windowBits to -31 to allow deflate raw  (-15 Allow Raw, -16
      // Value to convert the windowBits)
      wb = -31;
    }

    if (!isDeflate) {
      inflateSyncBuffer(isolate, flush, wb, self->chunkSize, inputLength, input,
                        r);
    } else {
      deflateSyncBuffer(isolate, flush, wb, Z_DEFAULT_COMPRESSION, 8,
                        Z_DEFAULT_STRATEGY, inputLength, input, r);
    }
  ...
  static void New(const v8::FunctionCallbackInfo<v8::Value> &args) {
    ...
    if (args.IsConstructCall()) {
      ...
      if (args[0]->IsString()) {
        ...
        if ((strcmp(method, "inflate") != NULL &&
             strcmp(method, "deflate") != NULL &&
        ...
        isolate->ThrowException(
            v8::Exception::TypeError(v8::String::NewFromUtf8Literal(
                isolate, "Expected a method for process")));
        return;
      }
      ...
      if (args[1]->IsObject()) {
        ...
        v8::Local<v8::Value> defaultFlushOption{
            options->Get(context, v8::String::NewFromUtf8Literal(
                                      isolate, "defaultFlush"))
                .ToLocalChecked()};
        if (defaultFlushOption->IsInt32()) {
          defaultFlush =
              defaultFlushOption->ToInt32(context).ToLocalChecked()->Value();
        }

        v8::Local<v8::Value> windowBitsOption{
            options->Get(context,
                         v8::String::NewFromUtf8Literal(isolate, "windowBits"))
                .ToLocalChecked()};
        if (windowBitsOption->IsInt32()) {
          windowBits =
              windowBitsOption->ToInt32(context).ToLocalChecked()->Value();
        }

        v8::Local<v8::Value> chunkSizeOption{
            options->Get(context,
                         v8::String::NewFromUtf8Literal(isolate, "chunkSize"))
                .ToLocalChecked()};
        if (chunkSizeOption->IsInt32()) {
          chunkSize =
              chunkSizeOption->ToInt32(context).ToLocalChecked()->Value();
        }
      }

      ProcessSync *self{
          new ProcessSync(method, windowBits, chunkSize, defaultFlush)};
      self->Wrap(args.This());
      args.GetReturnValue().Set(args.This());
    } else {
      isolate->ThrowException(v8::Exception::TypeError(v8::String::NewFromUtf8Literal(isolate, "Use the new operator to create instances of this object.")));
      return;
    }
  }

  static void GetChunkSize(v8::Local<v8::String> property,
  ...
  static void GetWindowBits(v8::Local<v8::String> property,
                            const v8::PropertyCallbackInfo<v8::Value> &args) {
    ProcessSync *self = ObjectWrap::Unwrap<ProcessSync>(args.This());
  ...
  static void GetDefaultFlush(v8::Local<v8::String> property,
                              const v8::PropertyCallbackInfo<v8::Value> &args) {
    ProcessSync *self = ObjectWrap::Unwrap<ProcessSync>(args.This());
  ...
  static void GetResult(v8::Local<v8::String> property,
                        const v8::PropertyCallbackInfo<v8::Value> &args) {
    ProcessSync *self = ObjectWrap::Unwrap<ProcessSync>(args.This());
    ...
    if (self->result.size() == 0) {
      ...
      v8::Local<v8::Object> resultBuffer =
          node::Buffer::Copy(args.GetIsolate(), self->result.data(),
                             self->result.size())
              .ToLocalChecked();
  ...
  static void Init(v8::Local<v8::Object> exports) {
    ...
    iTpl->SetAccessor(v8::String::NewFromUtf8Literal(isolate, "chunkSize"),
                      GetChunkSize);
    iTpl->SetAccessor(v8::String::NewFromUtf8Literal(isolate, "windowBits"),
                      GetWindowBits);
    iTpl->SetAccessor(v8::String::NewFromUtf8Literal(isolate, "defaultFlush"),
                      GetDefaultFlush);
    iTpl->SetAccessor(v8::String::NewFromUtf8Literal(isolate, "result"),
                      GetResult);
    ...
    exports->Set(context,
                 v8::String::NewFromUtf8Literal(isolate, "ProcessSync"),
                 constructor);
  }
};
...
#define SET_CONSTANT(prop)                                                     \
  ...
  exports->Set(context, v8::String::NewFromUtf8Literal(isolate, "constants"),
               constants);
