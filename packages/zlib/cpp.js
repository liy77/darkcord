...
const inflate = exports.inflate = (buffer, options = {}) => {
    return zlib.inflate(buffer, options.flush, options.windowBits, options.chunkSize);
}

const deflate = exports.deflate = (buffer, options = {}) => {
    return zlib.deflate(buffer, options.flush, options.strategy, options.level, options.windowBits, options.memoryLevel);
}
...
exports.createDeflateRawProcess = (options) => new zlib.ProcessSync("deflateRaw", options);
