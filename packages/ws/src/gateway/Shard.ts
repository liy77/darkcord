  ...
  try {
    ...
    try {
      ...
      throw MakeError({
        name: "NoZlib",
        message:
          "Missing zlib package, please install @darkcord/zlib or zlib-sync",
      });
    }
  }
}
...
export class GatewayShard extends EventEmitter {
   ...
   */
  status: Constants.GatewayStatus;
  /**
   * Queue is processing
   */
  ...
  #onMessage(message: WebSocket.MessageEvent) {
    try {
      ...
      if (this.options.compress) {
        // Get zlib package, zlib-sync or @darkcord/zlib
        getZlib();
